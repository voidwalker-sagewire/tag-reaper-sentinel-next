...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        seekAnt1Power = findViewById(R.id.seekAnt1Power);
        seekAnt2Power = findViewById(R.id.seekAnt2Power);
        seekAnt3Power = findViewById(R.id.seekAnt3Power);
        seekAnt4Power = findViewById(R.id.seekAnt4Power);

        txtAnt1Power = findViewById(R.id.txtAnt1Power);
        txtAnt2Power = findViewById(R.id.txtAnt2Power);
        txtAnt3Power = findViewById(R.id.txtAnt3Power);
        txtAnt4Power = findViewById(R.id.txtAnt4Power);

        setupPowerControl(seekAnt1Power, txtAnt1Power, 30);
        setupPowerControl(seekAnt2Power, txtAnt2Power, 30);
        setupPowerControl(seekAnt3Power, txtAnt3Power, 30);
        setupPowerControl(seekAnt4Power, txtAnt4Power, 30);
    ...
    private void startOrResumeScanning() {
        ...
        new Thread(() -> {
            String powerError = applyAndVerifyAntennaPower();
            if (powerError != null) {
                final String finalPowerError = powerError;
                runOnUiThread(() -> {
                    setStatus(finalPowerError);
                    updateControls();
                });
                return;
            }
    ...
    private void setupPowerControl(SeekBar seekBar, TextView label, int defaultDbm) {
        seekBar.setMax(MAX_POWER_DBM - MIN_POWER_DBM);
        seekBar.setProgress(defaultDbm - MIN_POWER_DBM);
        label.setText(defaultDbm + " dBm");
        seekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            @Override
            public void onProgressChanged(SeekBar bar, int progress, boolean fromUser) {
                label.setText((progress + MIN_POWER_DBM) + " dBm");
            }
    ...
    private int selectedPower(SeekBar seekBar) {
        return seekBar.getProgress() + MIN_POWER_DBM;
    }
    ...
    private String applyAndVerifyAntennaPower() {
        try {
            ...
            SeekBar[] controls = {seekAnt1Power, seekAnt2Power, seekAnt3Power, seekAnt4Power};
            for (int i = 0; i < antennas.length; i++) {
                int requested = selectedPower(controls[i]);
                if (!reader.setAntennaPower(antennas[i], requested)) {
                    return "ANT " + (i + 1) + " power rejected";
                }
                int confirmed = reader.getAntennaPower(antennas[i]);
                if (confirmed != requested) {
                    return "ANT " + (i + 1) + " power mismatch: requested " + requested + ", confirmed " + confirmed;
                }
            }
            return null;
        } catch (Throwable throwable) {
            return "Power setup failed — " + throwable.getClass().getSimpleName() + ": " + throwable.getMessage();
        }
    }
